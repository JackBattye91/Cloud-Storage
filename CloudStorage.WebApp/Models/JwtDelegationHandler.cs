...
namespace CloudStorage.WebApp.Models
{
    public class JwtDelegationHandler : DelegatingHandler
    {
        ...
        public JwtDelegationHandler(ProtectedLocalStorage pProtectedLocalStorage)
        {
        ...
        private async Task GetRefreshToken(HttpRequestMessage pRequestMessage, CancellationToken cancellationToken)
        {
            try
            {
                ...
                else
                {
                    ...
                    string baseUrl = pRequestMessage.RequestUri.AbsoluteUri.Substring(0, baseUrlEnd).Trim('/');
